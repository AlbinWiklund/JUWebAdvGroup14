...
const ID_TOKEN_EXPIRATION_TIME_IN_S = 600
...
app.get('/account/:id/books/reviews', async function (request, response) {
	...
	try {
		...
		const accountInformation = [account, books, reviews, avgAccRating] // its an array of information needed for the account :)
...
app.get('/account/:id', async function (request, response) {
	...
	try {
		const query = "SELECT id, username, name, surname FROM accounts WHERE id = ?"
...
app.post('/account/create', async function (request, response) {
	...
	bcrypt.hash(account.password, saltRounds, async function (err, hash) {
		...
		const accountsUsername = await connection.query(usernameQuery, usernameValues)

		if (accountsUsername[0] != undefined) {
			response.status(400).json(["usernameAlreadyExists"])
			return
		}// Constraint has been added but does not want to actually work for some reason which is why the above query is still in place
		try {
			...
			jwt.sign(accessTokenPayload, ACCESS_TOKEN_SECRET, async function (error, accessToken) {
				if (error) {
					...
					const idTokenPayload = {
						...
						exp: Math.floor(Date.now() / 1000) + ID_TOKEN_EXPIRATION_TIME_IN_S,
...
app.post('/book/create', async function (request, response) {
	...
	jwt.verify(accessToken, ACCESS_TOKEN_SECRET, async function (error, payload) {
		if (error) {
			...
			try {
				if (payload.sub == request.body.accountID){
...
app.post('/book/:id/review', async function (request, response) {
	...
	jwt.verify(accessToken, ACCESS_TOKEN_SECRET, async function (error, payload) {
		if (error || payload.sub != review.reviewerId) {
			response.status(401).end()
		} else {
			const connection = await pool.getConnection()
			...
			try {
				...
				const parsedBookObject = JSON.parse(stringBookObject) // for some reason if this is not done it will crash
...
app.put('/account/:id', async function (request, response) {
	...
	jwt.verify(accessToken, ACCESS_TOKEN_SECRET, async function (error, payload) {
		if (error || payload.sub != account.accountId) {
			response.status(401).end()
		} else {
			const connection = await pool.getConnection()
...
app.put('/book/:id', async function (request, response) {
	...
	jwt.verify(accessToken, ACCESS_TOKEN_SECRET, async function (error, payload) {
		if (error || payload.sub != book.accountId) {
			response.status(401).end()
		} else {
			const connection = await pool.getConnection()
...
app.delete('/account/:id', async function (request, response) {
	...
	jwt.verify(accessToken, ACCESS_TOKEN_SECRET, async function (error, payload) {
		if (error || payload.sub != request.params.id) {
			response.status(401).end()
		} else {
			const connection = await pool.getConnection()
...
app.delete('/book/:id', async function (request, response) {
	...
	jwt.verify(accessToken, ACCESS_TOKEN_SECRET, async function (error, payload) {
		if (error) {
			...
			try {
				const accountIdQuery = "SELECT accountID AS accountID FROM books WHERE id = ?"
				const values = [request.params.id]
				const accIdFromBook = await connection.query(accountIdQuery, values)
				if (payload.sub != accIdFromBook[0]["accountID"]) {
					response.status(401).end()
				} else {
...
app.delete('/review/:id', async function (request, response) {
	...
	jwt.verify(accessToken, ACCESS_TOKEN_SECRET, async function (error, payload) {
		if (error) {
			...
			try {
				const accountIdQuery = "SELECT reviewerID AS accountID FROM books WHERE id = ?"
				const values = [request.params.id]
				const accIdFromReview = await connection.query(accountIdQuery, values)
				if (payload.sub != accIdFromReview[0]["accountID"]) {
					response.status(401).end()
				}
